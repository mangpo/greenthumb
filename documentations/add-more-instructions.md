...
The enumerative search requires more efforts. If we add **new argument type**, we will need to modify method `generate-inst` to generate instructions in `llvm-demo-enumerator.rkt` and method `interpret-inst` to interpret an instruction backward in `llvm-demo-inverse.rkt`. If you are at this stage, please contact `mangpo@eecs.berkeley.edu`. We are working on eliminating this step entirely if possible.
