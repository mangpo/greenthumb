...
(define arm-simulator-rosette%
  (class simulator%
    ...
    (define bvrev (bvop rev))
    (define bvrev16 (bvop rev16))
    (define bvrevsh (bvop revsh))
    (define bvrbit (bvop rbit))
    ...
    (define (sext d a width shift)
      (bitwise-ior
       (if (= (bitwise-bit-field a (sub1 width) width) 1)
           (shl -1 width)
           0)
       (bitwise-and (>> a shift) (sub1 (shl 1 width)))))
    ...
    (define (rbit a)
      (let ([res 0])
        (for ([i bit])
             (set! res (shl res 1) (bitwise-and a 1))
             (set! a (>> a 1 bit)))
        res))
    ...
    (define (interpret program state [is-candidate #f] #:dep [dep #f])
	  ...
	  (if dep
        ...
        (cond
         ...
         [(inst-eq `udiv) (rrr (lambda (x y) (quotient x (bitwise-and y mask))))]
         ...
         [(inst-eq `sbfx) (rrii ext)]
         [(inst-eq `ubfx) (rrii sext)]
         [(inst-eq `clz)  (rr clz)]

         ;; load/store
         [(inst-eq `ldri) (ldr (lambda (x y) (vector-ref memory (+ x y))))]
         [(inst-eq `stri) (str)]
         [(inst-eq `ldr)  (ldr (lambda (x y) (vector-ref memory (+ x y))))]
         [(inst-eq `str)  (str)]
    ...
    (define (performance-cost code)
      ...
      (for ([x code])
           (cond
            ...
            [(equal? (inst-op x) (vector-member `mov32 inst-id)) 
             (set! cost (+ cost 2))]
