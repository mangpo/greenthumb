...
(define arm-simulator-rosette%
  (class simulator%
    ...
    (define (interpret program state [is-candidate #f] #:dep [dep #f])
	  ...
	  (define (inst-reg-pattern f nr ni [rw? #f])
            (define d (args-ref args 0))
	    (define arg-list
	      (append
	       (if rw? (list (vector-ref args d)) (list))
	       (for/list ([i (range 1 nr)]) (vector-ref regs (args-ref args i)))
	       (for/list ([i (range ni)]) (args-ref args (+ nr i)))))
	    (define val (apply f arg-list))
	    (vector-set! regs d val)
	    (if dep
		(vector-set! regs-dep d (create-node 
					 val
					 (append (if rw? (list (vector-ref regs-dep d)) (list))
						 (for/list ([i (range 1 nr)]) 
							   (vector-ref regs-dep (args-ref args i))))))
		(add-inter val)))

          (define (rrrr f) (inst-reg-pattern f 4 0))
